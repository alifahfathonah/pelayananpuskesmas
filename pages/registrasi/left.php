...
$queryreg = $con->prepare("SELECT * FROM `antrian` WHERE ((`STATUS`='1' OR `STATUS`='11') AND (`TUJUANNO`='1') AND (`TANGGAL`='".$tglsekarang."')) ORDER BY ID ASC LIMIT 1");
$queryreg->execute();
...
$masterpoli = $con->prepare("SELECT * FROM `masterdata` WHERE `STATUS`='1'");
$masterpoli->execute();
...
<audio id="suarabelruangan1" src="../rekaman/registrasi.mp3" ></audio>
<div class="box box-default">
    <div class="box-header with-border">
      <i class="fa fa-medkit"></i>

      <h3 class="box-title">RUANG PENDAFTARAN</h3>
    </div>
    <!-- /.box-header -->
    <div class="box-body">
...
<div class="box box-default">
    <div class="box-header with-border">
      ...
      <input name="nama" id="nama" type="text" class="form-control input-lg" placeholder="Nama Pasien" style="text-transform: uppercase;"></input>
    </div>
    <!-- /.box-header -->
    <div class="box-body">
        ...
        <div class='btn-group pilihpoli'>
            <button class='btn btn-lg bg-purple'><i class='fa fa-send'></i> PILIH POLI</button>
            ...
            <ul class='dropdown-menu dropdown-info'>
                <?php
                  foreach ($datapoli as $rowpoli) {
                    echo "<li><a href='#' class='poli' id_antrian='".$idant1."' id_poli='".$rowpoli['ID_R']."' poli_nm='POLI ".$rowpoli['RUANG']."' nomor_antrian='".$number."'><i class='fa fa-user-md'></i><b> POLI ".$rowpoli['RUANG']."</b></a></li>";
                  }
                ?>
            </ul>
...
?>
      ...
      $(function() {
        $(".poli").click(function(){
            ...
            var app_nama =  document.getElementById('nama').value;               
            if(confirm("NOMOR ANTRIAN "+app_nomor+" AKAN MENUJU KE "+app_polinm+"\nKLIK OK JIKA BENAR.!")){
                    $.ajax({
                            type: "POST",
                            url: "procantrian.php",
                            data: 'id=' + app_id + '&idpoli=' + app_poli + '&idpolinm=' + app_polinm + '&namapas=' + app_nama,
                            success: function() {
                            }
                    });
                            location.reload();
