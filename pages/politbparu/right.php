...
$berikutnya = $con->prepare("SELECT NOMOR, JAM FROM `antrian` WHERE `TUJUANNO`='13' AND STATUS='1' AND TANGGAL='".$tanggal."' ORDER BY ID ASC LIMIT 5");
$berikutnya->execute();
$datab=$berikutnya->fetchAll();

$tunda = $con->prepare("SELECT ID, NOMOR, JAM FROM `antrian` WHERE `TUJUANNO`='13' AND STATUS='0' AND TANGGAL='".$tanggal."' ORDER BY ID ASC LIMIT 5");
$tunda->execute();
$datat=$tunda->fetchAll();

$masterpoli2 = $con->prepare("SELECT * FROM `masterdata` WHERE `STATUS`='2'");
$masterpoli2->execute();
              ...
              <tbody>                
                ...
                foreach ($datat as $row2) {
                    echo "<tr>";
                      ...
                      <td>
                        <div class='btn-group'>
                            <button class='btn btn-sm bg-purple'><i class='fa fa-send'></i> TINDAKAN</button>
                            ...
                            <ul class='dropdown-menu dropdown-info'>
                                <?php
                                  foreach ($datapoli2 as $rowpoli2) {
                                    echo "<li><a href='#' class='poli2' id_antrianT='".$row2['ID']."' id_poliT='".$rowpoli2['ID_R']."' poli_nmT='".$rowpoli2['RUANG']."' nomor_antrianT='".$row2['NOMOR']."'><i class='fa fa-user-md'></i><b> R ".$rowpoli2['RUANG']."</b></a></li>";
                                  }
                                ?>
                            </ul>
